...
import {
  useCreateSocietyCreditTypeMutation,
  useUpdateSocietyCreditTypeMutation,
...
const CreateOrEditCreditType = props => {
  ...
  const [
    createSocietyCreditTypeMutation,
    {
      loading: isCreateSocietyCreditTypeLoading,
      data: createSocietyCreditTypeData,
    },
  ] = useCreateSocietyCreditTypeMutation();

  const [
    updateSocietyCreditTypeMutation,
    {
      loading: isUpdateSocietyCreditTypeLoading,
      data: updateSocietyCreditTypeData,
    },
  ] = useUpdateSocietyCreditTypeMutation();
  ...
  const onCreateOrEditType = values => {
    if (type) {
      ...
      createSocietyCreditTypeMutation({
        variables: {
          createSocietyCreditTypeDto: values,
        },
      });
    }
  };
  ...
  useEffect(() => {
    if (createSocietyCreditTypeData || updateSocietyCreditTypeData) {
      navigation.goBack();
    }
  }, [createSocietyCreditTypeData, updateSocietyCreditTypeData]);
    ...
    <ScrollView keyboardShouldPersistTaps={KeyboardShouldPersistTypes.handled}>
      <View style={[CommonStyles.bigMarginBottom]}>
        <Text
          ...
          {t('Enter revenue type fields')}
        </Text>
        <Text>
          {t(
            'Please make sure to fill all required fields before adding revenue type',
          )}
        </Text>
      </View>
      <YupTextInput
        ...
        name={TypeKeys.name}
        errors={errors}
        placeholder={t('Revenue type name')}
      />
      <NetworkButton
        style={[
          {
        ...
        disabled={
          !isValid ||
          isCreateSocietyCreditTypeLoading ||
          isUpdateSocietyCreditTypeLoading
        }
        loading={
          isCreateSocietyCreditTypeLoading || isUpdateSocietyCreditTypeLoading
        }
...
export default CreateOrEditCreditType;
