...
import {useUpdateSocietyProfileMutation} from '@graphql/actions/society/mutations';
...
const EditSocietyProfile = props => {
  ...
  const schema = yup.object().shape({
    [UserKeys.avatar]: yup.object().required(t('Profile picture is required')),
  ...
  const [
    updateSocietyProfileMutation,
    {loading: isUpdateSocietyProfileLoading, data: updateSocietyProfileData},
  ] = useUpdateSocietyProfileMutation();

  const onUpdateSocietyProfile = values => {
    setRedirect(true);
    const pPicKey = values?.avatar?.uri?.replace(
      Config.awsS3BucketCloudFrontUri,
      CommonStrings.empty,
    );
    console.log({component: EditSocietyProfile.name, values, pPicKey});
    updateSocietyProfileMutation({
      variables: {
        updateSocietyProfileDto: {
          name: values?.name,
  ...
  useEffect(() => {
    if (updateSocietyProfileData && redirect) {
      navigation.goBack();
    }
  }, [updateSocietyProfileData]);
  ...
  return (
    <NetworkScrollView
      ...
      <NetworkButton
        ...
        disabled={!isValid || isUpdateSocietyProfileLoading}
        loading={isUpdateSocietyProfileLoading}
        onPress={handleSubmit(onUpdateSocietyProfile)}>
