...
import {useSignInMemberQuery} from '@graphql/actions/auth/queries';
...
const MemberSignIn = props => {
  ...
  const [
    signInMemberQuery,
    {loading: isSignInMemberLoading, data: signInMemberData},
  ] = useSignInMemberQuery();

  const onSignInMember = values => {
    signInMemberQuery({variables: {signInMemberDto: values}});
  };

  const [isLoading, setIsLoading] = useState(false);

  useEffect(() => {
    const login = async signInMemberData => {
      if (signInMemberData) {
        try {
          setIsLoading(true);
          const response = signInMemberData?.response;
          ...
          console.log({component: MemberSignIn.name, e});
        }
      }
    };
    login(signInMemberData);
  }, [signInMemberData, isSignInMemberLoading]);
  ...
  return (
    <View style={[{padding: Paddings.big}]}>
      <View style={[CommonStyles.bigMarginBottom]}>
        ...
        <Text>
          {t('SignIn and track your all the society account information')}
        </Text>
      </View>
      <View style={[CommonStyles.bigMarginBottom]}>
        <YupTextInput
        ...
        <TextButton
          ...
          onPress={() => {
            navigation.navigate(RouteNames.ForgotPassword, {
              [UserKeys.userType]: UserTypes.Member,
              disableRememberMe,
            });
          }}
        />
      ...
      <View style={[CommonStyles.bigMarginBottom]}>
        <NetworkButton
          ...
          disabled={!isValid || isSignInMemberLoading || isLoading}
          loading={isSignInMemberLoading || isLoading}
          onPress={handleSubmit(onSignInMember)}>
